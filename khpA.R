...
sampleFiles <- list.files(path=directory, pattern='WT|khpA')

# Filter out files that match the exclude pattern
sampleFiles <- sampleFiles[!grepl('ALL_merged', sampleFiles)]
sampleFiles

sampleNames <- c('khpA_1', 'khpA_2', 'khpA_3', 'WT_1', 'WT_2', 'WT_3')

OD = c(1, 1, 1, 2, 2, 1)

Names <- c('khpA_1', 'khpA_2', 'khpA_3', 'WT_1', 'WT_2', 'WT_3')

Strain <- c('khpA', 'khpA', 'khpA', 'WT', 'WT', 'WT')
...
colData(dds)$strain<-factor(colData(dds)$strain, levels=c('WT','khpA'))
...
output_file <- file.path(output_dir, "PCA_plot_khpA_all_replicates.pdf")
# Save the plot as a PDF file
ggsave(output_file, plot = pca_plot, width = 8, height = 6)


### khpA_1 seem to be an outlier
### remove and refit model
...
# Specify the output directory and file name
output_dir <- "/Users/andreas/Bacillus/Bioinformatics/RNAseq_jag_khpa_kre/X204SC22071444-Z01-F001/Analysis/figures/"
output_file <- file.path(output_dir, "PCA_plot_khpA_without_rep1.pdf")
# Save the plot as a PDF file
ggsave(output_file, plot = pca_plot, width = 8, height = 6)
...
res <- results(dds, contrast = c("strain", "khpA", "WT")) 
...
# Specify the output directory and file name
output_dir <- "/Users/andreas/Bacillus/Bioinformatics/RNAseq_jag_khpa_kre/X204SC22071444-Z01-F001/Analysis/figures/"
output_file <- file.path(output_dir, "Volcano_plot_khpA.pdf")
# Save the plot as a PDF file
...
output_file <- file.path(output_dir, "khpA_DEG.txt")
...
### RNA-seq
gene_list_file <- "/Users/andreas/Bacillus/Bioinformatics/RNAseq_jag_khpa_kre/X204SC22071444-Z01-F001/Analysis/DEG_significant/khpA_DEG.txt_sig_genes_BSU_Numbers.txt"
...
### CRAC
# Specify the path to your gene list file
gene_list_file <- "/Users/andreas/Bacillus/Bioinformatics/CRAC_jag_khpA_kre_19thJune2024/ratios/prepared/khpA_ratios.txt_sig_genes_BSU_Numbers.txt"
# Read the gene list into R
genes_of_interest <- read.table(gene_list_file, header = FALSE, stringsAsFactors = FALSE)
genes_of_interest <- as.character(genes_of_interest$V1)
...
output_file <- file.path(output_dir, "Dotplot_GSEA_khpA_CRAC.pdf")
# Save the plot as a PDF file
...
# Specify the path to your gene list file
gene_list_file <- "/Users/andreas/Bacillus/Bioinformatics/RNAseq_jag_khpa_kre/X204SC22071444-Z01-F001/Analysis/DEG_significant/khpA_DEG.txt_sig_genes_BSUNumbers_no_underscore.txt"
# Read the gene list into R
genes_of_interest <- read.table(gene_list_file, header = FALSE, stringsAsFactors = FALSE)
genes_of_interest <- as.character(genes_of_interest$V1)
...
# Specify the output directory and file name
output_dir <- "/Users/andreas/Bacillus/Bioinformatics/RNAseq_jag_khpa_kre/X204SC22071444-Z01-F001/Analysis/figures/"
output_file <- file.path(output_dir, "Dotplot_KEGG_khpA.pdf")
# Save the plot as a PDF file
